...
/* Веб-страница для управления светодиодом */
static esp_err_t index_html_handler(httpd_req_t *req) {
  const char *html_page =
      ...
      "</head>"
      "<body>"
      "    <h1>🎛️ ESP32 LED Control</h1>"
      "    <p class='status' id='status'>LED status: Unknown</p>"
      "    <button class='btn on' onclick='controlLED(\"on\")'>🔵 LED "
      "ON</button>"
      "    <button class='btn off' onclick='controlLED(\"off\")'>⚪ LED "
      "OFF</button>"
      "    <script>"
      "        async function controlLED(state) {"
      "            const response = await fetch('/led/' + state, { method: "
      "'POST' });"
      "            const text = await response.text();"
      "            document.getElementById('status').innerHTML = 'LED status: "
      "' + text;"
      "        }"
      ...
      "                .then(r => r.text())"
      "                .then(text => {"
      "                    document.getElementById('status').innerHTML = 'LED "
      "status: ' + text;"
      "                });"
      "        }"
      "    </script>"
...
/* Обработчик для включения LED */
static esp_err_t led_on_handler(httpd_req_t *req) {
  // Здесь будет включение GPIO
...
/* Обработчик статуса LED */
static esp_err_t led_status_handler(httpd_req_t *req) {
  // Здесь можно проверить состояние GPIO
  // int level = gpio_get_level(LED_PIN);
  httpd_resp_send(req, "UNKNOWN (GPIO not configured)", HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}
...
static const httpd_uri_t index_html = {.uri = "/",
                                       .method = HTTP_GET,
                                       .handler = index_html_handler,
                                       .user_ctx = NULL};

static const httpd_uri_t led_on = {.uri = "/led/on",
                                   .method = HTTP_POST,
                                   .handler = led_on_handler,
                                   .user_ctx = NULL};

static const httpd_uri_t led_off = {.uri = "/led/off",
                                    .method = HTTP_POST,
                                    .handler = led_off_handler,
                                    .user_ctx = NULL};

static const httpd_uri_t led_status = {.uri = "/led/status",
                                       .method = HTTP_GET,
...
static httpd_handle_t start_webserver(void) {
  ...
  if (httpd_start(&server, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(server, &index_html); // Главная страница
    httpd_register_uri_handler(server, &led_on);     // Включить LED
    httpd_register_uri_handler(server, &led_off);    // Выключить LED
    httpd_register_uri_handler(server, &led_status); // Статус LED
