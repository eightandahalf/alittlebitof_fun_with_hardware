...
static void http_rest_with_url(void) {
  ...
  esp_http_client_set_url(client,
                          "http://" CONFIG_EXAMPLE_HTTP_ENDPOINT "/post");
  ...
  // PUT
  esp_http_client_set_url(client,
                          "http://" CONFIG_EXAMPLE_HTTP_ENDPOINT "/put");
  ...
  // PATCH
  esp_http_client_set_url(client,
                          "http://" CONFIG_EXAMPLE_HTTP_ENDPOINT "/patch");
  ...
  // DELETE
  esp_http_client_set_url(client,
                          "http://" CONFIG_EXAMPLE_HTTP_ENDPOINT "/delete");
...
static void http_auth_basic(void) {
   ...
   */
  esp_http_client_config_t config = {
      .url = "http://user:passwd@" CONFIG_EXAMPLE_HTTP_ENDPOINT
...
static void http_auth_digest_sha256(void) {
  esp_http_client_config_t config = {
      .url = "http://user:passwd@" CONFIG_EXAMPLE_HTTP_ENDPOINT
             "/digest-auth/auth/user/passwd/SHA-256/never",
      .event_handler = _http_event_handler,
      .buffer_size_tx = 1024, // Increase buffer size as header size will
                              // increase as it contains SHA-256.
  };
  ...
  if (err == ESP_OK) {
    ESP_LOGI(TAG,
             "HTTP SHA256 Digest Auth Status = %d, content_length = %" PRId64,
             esp_http_client_get_status_code(client),
             esp_http_client_get_content_length(client));
  } else {
    ESP_LOGE(TAG, "Error performing http request %s", esp_err_to_name(err));
...
static void http_encoded_query(void) {
  ...
  uint32_t enc_len =
      example_uri_encode(query_val_enc, query_val, strlen(query_val));
  if (enc_len > 0) {
...
static void http_absolute_redirect_manual(void) {
  ...
  if (err == ESP_OK) {
    ESP_LOGI(TAG,
             "HTTP Absolute path redirect (manual) Status = %d, content_length "
             "= %" PRId64,
             esp_http_client_get_status_code(client),
             esp_http_client_get_content_length(client));
  } else {
...
static void https_async(void) {
  ...
  const char *post_data =
      ...
      "be used by the Dúnedain to communicate throughout the Realms in Exile. "
      "During the War of the Ring, "
      "the Palantíri were used by many individuals. Sauron used the "
      "Ithil-stone to take advantage of the users "
      "of the other two stones, the Orthanc-stone and Anor-stone, but was also "
